...
2. 基本パッケージ導入（`neovim`, `bat`, `fzf`, `colordiff`, `eza` を含む軽量構成）
3. `starship` 未導入時のみインストール（公式 install script）
4. `settings/common/` + `settings/wsl/` をホームへ反映
...
6. `~/.venv-tools` を `uv venv` で作成（未作成時）
7. `uv pip install` で以下を導入
   - `pandas`, `numpy`, `matplotlib`, `requests`, `rich`, `tqdm`, `ipython`
8. 可能なら default shell を `zsh` に変更
